...
public class D_carbon_friction_receive_detail_2DAO {
    ...
    public void insert(DataBeanD_carbon_friction_receive_detail_2 DataBean) throws Exception {
        ...
        int i = 1;
        try {
            p = con.prepareStatement("insert into d_carbon_friction_receive_detail_2(doc_id,line_no,product_id,bag_no,location_id,amount_bag,weight,total_weight,wh_in,create_by,create_date,update_date,doc_date,location_id2) "
                    + "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            ...
            i += 1;
            p.setString(i, DataBean.getBag_no());
            //p.setString(i, DataBean.getLine_no());
            i += 1;
            p.setString(i, DataBean.getLocation_id());
            i += 1;
            p.setString(i, DataBean.getAmount_bag());
            i += 1;
            ...
            i += 1;
            p.setString(i, DataBean.getLocation_id_ref());
            i += 1;

            String SqlCount = "Select Count(doc_id) As num from  d_carbon_friction_receive_detail_2 where doc_id = '" + DataBean.getDoc_id() + "' and delete_flag = 'N' and line_no = '" + DataBean.getLine_no() + "' ;";
    ...
    public void update(DataBeanD_carbon_friction_receive_detail_2 DataBean) throws Exception {
        ...
        try {
            p = con.prepareStatement("update d_carbon_friction_receive_detail_2 set "
                    + "product_id=?,bag_no=?,location_id=?,amount_bag=?,weight=?,total_weight=?,wh_in=?,update_by=?,update_date=?,doc_date=?,location_id2=? "
    ...
    public DataBeanD_carbon_friction_receive_detail_2 ReturnEdit(String doc_id, String line_no) throws Exception {
        ...
        String SQL = "Select product_id,doc_date,bag_no,location_id,amount_bag,weight,total_weight,wh_in from d_carbon_friction_receive_detail_2 where doc_id ='" + doc_id + "' and line_no = '" + line_no + "' and delete_flag = 'N' ";
        try {
    ...
    public String ShowDetail(String doc_id) throws Exception {
        ...
        String SQL = "select count(doc_id) as num from d_carbon_friction_receive_detail_2 where doc_id = '" + doc_id + "' and delete_flag = 'N' ";
        try {
            ...
            if (uti.numRowdatabase(SQL) == 0) {
                ...
                SQL = "select runno,doc_date,doc_id,line_no,product_id,bag_no,location_id,amount_bag,weight,total_weight,wh_in from d_carbon_friction_receive_detail_2 where doc_id = '" + doc_id + "' and delete_flag = 'N' order by to_number(line_no,'990')";
                rs = con.createStatement().executeQuery(SQL);
                while (rs.next()) {
