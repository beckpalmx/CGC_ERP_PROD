...
<body>
    <form name="serForm" method="get" action="Search_Report2.jsp">
                ...
                <div class="panel-footer">        
                    <td  colspan="2" class="fcentercenter">
                        <table cellpadding="0" cellspacing="0" >
                            ...
                            <tr>
                                <td colspan="4" class="header2"><div align="left">รหัส</div></td>
                            ...
                            %>
                    <tr>
                        <td colspan="4" align="left"><a  href="#" onClick="gettoShow('<%=getshowdata%>', '<%=sentToshow%>')" class="link_transparent"><%=getshowdata%></a></td>
                    ...
                    <div align="center">
                        <%                    if (screen > 1) {
                                if (request.getParameter("searchid") == null) {%>
                        <a href="../SEARCHSCREEN/Search_Report2.jsp?screen=<%=screen - 1%>&table=<%=request.getParameter("table")%>&sentdataToshow=<%=sentToshow%>" class="link_transparent">ย้อนกลับ</a>
                        <%
                        } else if (request.getParameter("searchid") != null) {%>
                        <a href="../SEARCHSCREEN/Search_Report2.jsp?screen=<%=screen - 1%>&table=<%=request.getParameter("table")%>&searchid=<%=new String(request.getParameter("searchid").getBytes("ISO8859-1"), "TIS-620")%>&sentdataToshow=<%=sentToshow%>" class="link_transparent">ย้อนกลับ</a>
                        <%
                                }
                        ...
                        <%
                            ...
                            if (request.getParameter("searchid") == null) {%>
                        | <a href="../SEARCHSCREEN/Search_Report2.jsp?screen=<%=i%>&table=<%=request.getParameter("table")%>&sentdataToshow=<%=sentToshow%>" class="link_transparent"><%=i%></a> |
                        <%
                        } else if (request.getParameter("searchid") != null) {%>
                        | <a href="../SEARCHSCREEN/Search_Report2.jsp?screen=<%=i%>&table=<%=request.getParameter("table")%>&searchid=<%=new String(request.getParameter("searchid").getBytes("ISO8859-1"), "TIS-620")%>&sentdataToshow=<%=sentToshow%>" class="link_transparent"><%=i%></a> |
                        <%
                                    }
                                }
                        ...
                        <%
                            ...
                            if (request.getParameter("searchid") == null) {%>
                        | <a href="../SEARCHSCREEN/Search_Report2.jsp?screen=<%=y%>&table=<%=request.getParameter("table")%>&sentdataToshow=<%=sentToshow%>" class="link_transparent"><%=y%></a> |
                        <%
                        } else if (request.getParameter("searchid") != null) {%>
                        | <a href="../SEARCHSCREEN/Search_Report2.jsp?screen=<%=y%>&table=<%=request.getParameter("table")%>&searchid=<%=new String(request.getParameter("searchid").getBytes("ISO8859-1"), "TIS-620")%>&sentdataToshow=<%=sentToshow%>" class="link_transparent"><%=y%></a> |
                        <%
                                        }
                                    }
                                }
                            ...
                            if (screen < total_page) {
                                if (request.getParameter("searchid") == null) {%>
                        <a href="../SEARCHSCREEN/Search_Report2.jsp?screen=<%=screen + 1%>&table=<%=request.getParameter("table")%>&sentdataToshow=<%=sentToshow%>" class="link_transparent">ไปข้างหน้า</a>
                        <%
                        } else if (request.getParameter("searchid") != null && request.getParameter("searchtxt") != null) {%>
                        <a href="../SEARCHSCREEN/Search_Report2.jsp?screen=<%=screen + 1%>&table=<%=request.getParameter("table")%>&searchid=<%=new String(request.getParameter("searchid").getBytes("ISO8859-1"), "TIS-620")%>&sentdataToshow=<%=sentToshow%>" class="link_transparent">ไปข้างหน้า</a>
                        <%
                                    }
                                } else {
                                    ;
